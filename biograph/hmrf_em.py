...
class hmrf():
    ...
    def run(self):
        ...
        while (stopcrit_mu >= self.epsilon) or (stopcrit_cov >= self.epsilon) or cpt > 100:
            new_mu, new_cov = self.maximisation_step()
            
            # Stopping criteria
            stopcrit_mu = np.max(new_mu - self.mu)/(1 + np.max([new_mu, self.mu]))
            stopcrit_cov = np.max(new_cov - self.cov)/(1 + np.max([new_cov, self.cov]))
