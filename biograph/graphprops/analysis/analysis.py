...
def Energy_cells(G, Egg=1, Egr=1.5, Err=1):
    ...
    for edge in list(G.edges()):
        ...
        elif (colors[edge[0]] == "g" and colors[edge[1]] == "r") or (
            colors[edge[0]] == "r" and colors[edge[1]] == "g"
        ):
            E = E + Egr
...
def mean_lengths(G):
    ...
    return (
        total_length / len(G.edges),
        total_length_gg / nb_gglinks,
        total_length_rr / nb_rrlinks,
        total_length_gr / nb_grlinks,
    )
...
def surface_of_facet(facet, nodes, dsph):
    ...
    if m > 2:
        ...
        v1 = (nodes[facet[1]] - nodes[facet[0]]) / distance.euclidean(
            nodes[facet[1]], nodes[facet[0]]
        )
...
def get_volume_and_surfaces2(G):
    ...
    vol = hull.volume
    dsph = (3 * vol / (4 * math.pi)) ** (
        1 / 3
    )  # the radius of the spheroid considering that it is a sphere
    ...
    if len(regions) == len(G) + 1:
        ...
        for ind in vor.point_region:
            ...
            if (np.any(regions[ind] < 0) and n > 4) or (
                np.all(regions[ind] > 0) and n > 3
            ):
                for i in range(n):
                    if regions[ind][i] != -1:
        ...
        for edge in list(G.edges()):
            ...
            common_facet = list(
                set(ridge_vertices[edge[0]]).intersection(ridge_vertices[edge[1]])
            )
            if -1 in common_facet:
                ...
                G[edge[0]][edge[1]]["Area"] = surface_of_facet(
                    common_facet, nodes, dsph
                )
        return G
